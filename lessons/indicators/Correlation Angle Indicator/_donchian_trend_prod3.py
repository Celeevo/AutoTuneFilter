...
#   - Выход      : выбирается параметром exit_mode:
#                  1 = chandelier trailing-stop + reverse Donchian;
#                  2 = static stop-loss + take-profit в R от риска сделки.
...
class DonchianTrendStrategy(bt.Strategy):
    ...
    Trend-following:
      ...
      Exit mode 1: chandelier trailing-stop = max_high_since_entry - K*ATR
                   ИЛИ reverse Donchian: close < Donchian-low(donchian_exit, shift -1).
      Exit mode 2: static stop-loss + take-profit:
                   TP = entry +/- tp_mult * abs(entry - stop).
    ...
    params = dict(
        ...
        chandelier_atr_mult=3.0,       # K для initial stop и chandelier trailing-stop

        # === Exit mode ===
        # 1 — текущая логика: reverse Donchian + chandelier trailing-stop.
        # 2 — логика take-profit в R: static stop-loss + limit TP,
        #     где TP = entry +/- tp_mult * abs(entry - stop).
        exit_mode=1,
        tp_mult=2.0,                   # тейк-профит в R для exit_mode=2
    ...
    def __init__(self):
        ...
        if int(self.p.exit_mode) not in (1, 2):
            raise ValueError('exit_mode должен быть 1 или 2')
    ...
    def _calc_entry_params(self, isbuy):
        ...
        if comminfo.p.margin and comminfo.p.margin > 0:
    ...
    def _has_active_orders(self):
        # stop_order обычно живёт только вместе с открытой позицией, но
        # take_profit_order в режиме 2 тоже нужно учитывать, чтобы не открыть
        # новую сделку, пока старые выходные заявки ещё не сняты брокером.
        for o in (self.entry_order, self.close_order, self.stop_order, self.take_profit_order):
    ...
    def _place_stop_after_fill(self, executed_price):
        ...
        order = method(size=size, exectype=bt.Order.Stop,
                       price=self.stop_loss_price, _checksubmit=False)
        ...
        # В режиме 2 сразу после стопа выставляем take-profit как OCO-заявку.
        # В режиме 1 TP не нужен: там позиция сопровождается chandelier +
        # reverse Donchian.
        if int(self.p.exit_mode) == 2:
            self._place_take_profit_after_fill(executed_price)

    def _place_take_profit_after_fill(self, executed_price):
        """Выставляет take-profit в R от фактического entry-stop расстояния."""
        if self.stop_order is None:
            self.log('WARNING: take-profit не выставлен, потому что stop_order отсутствует')
            return

        comminfo = self.broker.getcommissioninfo(self.data)
        isbuy = self.entry_isbuy
        risk_points = abs(float(executed_price) - float(self.stop_loss_price))
        if risk_points <= 0:
            self.log('WARNING: risk_points <= 0, take-profit не выставлен')
            return

        direction = 1 if isbuy else -1
        raw_take_profit = float(executed_price) + direction * float(self.p.tp_mult) * risk_points
        self.take_profit_price = self._round_price(comminfo, raw_take_profit, isbuy)
        ...
        order.addinfo(name='take_profit')
        self.take_profit_order = order
        self.log(
            f'TAKE PROFIT placed at {self.take_profit_price:.4f} | '
            f'TP({float(self.p.tp_mult):.2f}R) | risk_points={risk_points:.4f}'
        )
    ...
    def _move_stop_to(self, new_stop_price):
        ...
        order = method(size=size, exectype=bt.Order.Stop,
                       price=new_stop_price, _checksubmit=False)
        order.addinfo(name='stop_loss')
        self.stop_order = order
    ...
    def _close_by_donchian(self):
        ...
        if self.stop_order is not None and self.stop_order.alive():
            self.cancel(self.stop_order)
        if self.take_profit_order is not None and self.take_profit_order.alive():
            self.cancel(self.take_profit_order)
        size = abs(self.position.size)
        method = self.sell if self.position.size > 0 else self.buy
        order = method(size=size, exectype=bt.Order.Market, _checksubmit=False)
    ...
    def notify_order(self, order):
        ...
        # Важно: Backtrader в notify_order может отдавать не тот же самый
        # Python-объект, который вернул buy()/sell(), а его clone/copy.
        # Поэтому identity-проверка `order is self.entry_order` недостаточна:
        # входной ордер может исполниться как UNKNOWN, стоп/TP не будут выставлены,
        # и в exit_mode=2 позиция останется висеть до конца теста без закрытой сделки.
        # Основная классификация — по стабильному order.info.name; identity оставлена
        # как дополнительная защита для случаев без info.name.
        is_entry = (order_name in ('long', 'short')) or (order is self.entry_order)
        is_stop = (order_name == 'stop_loss') or (order is self.stop_order)
        is_close = (order_name == 'close_donchian') or (order is self.close_order)
        is_take_profit = (order_name == 'take_profit') or (order is self.take_profit_order)
        ...
        if order.status == order.Completed:
            if is_entry:
                ...
                self.log(f'STOP LOSS EXECUTED at {order.executed.price:.4f}')
                if self.take_profit_order is not None and self.take_profit_order.alive():
                    self.cancel(self.take_profit_order)
                self._reset_state()
            elif is_take_profit:
                self.log(f'TAKE PROFIT EXECUTED at {order.executed.price:.4f}')
                if self.stop_order is not None and self.stop_order.alive():
                    self.cancel(self.stop_order)
                self._reset_state()
            elif is_close:
            ...
            if is_entry:
                ...
                # Cancel ожидаем при перевыставлении текущего стопа, OCO
                # take-profit или при close_donchian. Ссылку очищаем только
                # если отменён именно текущий объект; старые clone-уведомления
                # после перевыставления не должны затирать новый стоп.
                if order is self.stop_order:
                    self.stop_order = None
            elif order_name == 'take_profit':
                # Cancel ожидаем при OCO stop-loss или ручном закрытии.
                if order is self.take_profit_order:
                    self.take_profit_order = None
    ...
    def next(self):
        # Позиция открыта: способ сопровождения выбирается через exit_mode.
        if self.position:
            exit_mode = int(self.p.exit_mode)
            isbuy = self.entry_isbuy

            if exit_mode == 1:
                # Текущая логика PROD1: reverse Donchian имеет приоритет;
                # если он сработал, двигать стоп уже бессмысленно.
            ...
            if exit_mode == 2:
                # Логика из ATF-скрипта: static stop-loss + take-profit в R.
                # Здесь не используем reverse Donchian и не трейлим стоп,
                # чтобы сравнение режимов выхода было чистым.
                return
...
def main(maxcpus=None, exit_mode_override=None, tp_mult_override=None):
    start_cash = 300000.0
    ...
    params = dict(  # MIX
        write_history=False,
        risk=5,
        donchian_entry=[20, 30],  # #[20, 25, 30, 35],  #20,         # для оптимизации: [10, 15, 20, 25, 30, 40, 55]
        donchian_exit=8, #[8, 10, 12, 14], #10,          # для оптимизации: [5, 7, 10, 14, 20]
        ema_fast=10,
        ema_slow=40,
        atr_period=[10, 12, 14],
        ...
        chandelier_atr_mult=1.25,  #[i/4 for i in range(1, 8)], #3.0,   # для оптимизации: [i/2 for i in range(4, 9)]

        # === Exit mode ===
        # 1 = текущая логика PROD1: reverse Donchian + chandelier trailing-stop
        # 2 = static stop-loss + take-profit в R: TP = entry +/- tp_mult * risk_points
        # Для A/B-сравнения можно поставить exit_mode=[1, 2]
        exit_mode=2,
        tp_mult=[1.6, 1.7, 1.8, 1.9, 2.0, 2.2, 2.4, 2.6],  #2.0,  #
    ...
    # Можно выбрать режим выхода прямо при запуске скрипта:
    #   python donchian_trend_prod3.py --exit-mode 2 --tp-mult 2.0
    # Если аргументы не заданы, используются значения из params выше.
    if exit_mode_override is not None:
        params['exit_mode'] = exit_mode_override
    if tp_mult_override is not None:
        params['tp_mult'] = tp_mult_override
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Donchian trend strategy optimizer with selectable exit mode.'
    )
    parser.add_argument(
        '--exit-mode',
        type=int,
        choices=(1, 2),
        default=None,
        help='1 = reverse Donchian + chandelier trailing-stop; 2 = static stop-loss + take-profit in R',
    )
    parser.add_argument(
        '--tp-mult',
        type=float,
        default=None,
        help='Take-profit multiplier in R for exit_mode=2',
    )
    ...
    main(
        available_cpus,
        exit_mode_override=args.exit_mode,
        tp_mult_override=args.tp_mult,
    )
'''
    params = dict(  # Si
        write_history=False,
