...
futures_comm = FuturesCommission(
    ...
    moexcomm=0.00462,
    cost_of_price_step=0.001,  # шаг цены
)
...
def main(maxcpus=None):
    ...
    params = dict(
        depo=300000.0,
        risk=5,
        window=28,  #range(27,34),
        bandwidth=[i / 100 for i in range(45, 50)], #[0.4, 0.45, 0.45],
        thresh=-0.7,
        allow_short=True,
        tp_mult=[i / 10 for i in range(17, 23)],  #[1.7, 1.8, 1.9, 2],
    )
