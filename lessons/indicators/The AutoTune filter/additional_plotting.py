...
def save_best_equity_dd_plot(
    ...
    Ожидается, что первая колонка results_df содержит строку параметров,
    а trades_df содержит колонку params с тем же значением.
    """
    if trades_df is None or trades_df.empty:
        return None
    ...
    best_param_value = results_df.iloc[0, 0]
    trades_subset = trades_df[trades_df["params"] == best_param_value].copy()
    ...
    title = f"Equity / DD | {best_param_value}"
