...
def aggregate_results(df):
    """
    Собирает итоговые результаты оптимизации по вариантам параметров.

    Важно:
    каждая строка df — это результат одного прогона стратегии
    на одном фьючерсном контракте.

    Но один и тот же вариант параметров может встречаться
    много раз — по одному разу для каждого контракта.
    ...
    # Собираем итоговую таблицу и сортируем по PnL
    table = pd.DataFrame(rows)
    return table.sort_values(by='PnL', ascending=False).reset_index(drop=True)


def print_optimization_table(table):
    """Печатает аккуратную таблицу в терминал."""
...
def main(maxcpus=None):
    ...
    # и печатаем итоговую таблицу в терминал
    # --------------------------------------------------------
