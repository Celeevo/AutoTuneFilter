...
def _safe_float(v):
    """Возвращает float(v) или None, если v — None / NaN / что-то не приводится."""
    if v is None:
        return None
    try:
        f = float(v)
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            ...
            if self.strategy.p.write_history:
                ...
                # Диагностические метрики, зафиксированные стратегией в момент
                # сигнала входа. Если позиция закрылась раньше, чем стратегия
                # успела их записать (теоретически возможно на самых первых
                # сделках), берём None.
                trade.entry_ma_slope = getattr(self.strategy, '_entry_ma_slope', None)
                trade.entry_adx      = getattr(self.strategy, '_entry_adx', None)
                trade.entry_dc       = getattr(self.strategy, '_entry_dc', None)
                trade.entry_atr_pct  = getattr(self.strategy, '_entry_atr_pct', None)
        ...
        if self.strategy.p.write_history:
            ...
            for trade in self.trades:
                ...
                tr = dict(
                    ...
                    # === Диагностика на момент сигнала входа ===
                    entry_ma_slope=_safe_float(getattr(trade, 'entry_ma_slope', None)),
                    entry_adx=_safe_float(getattr(trade, 'entry_adx', None)),
                    entry_dc=_safe_float(getattr(trade, 'entry_dc', None)),
                    entry_atr_pct=_safe_float(getattr(trade, 'entry_atr_pct', None)),
                    # ===========================================
...
class AutoTuneFilterStrategy(bt.Strategy):
    ...
    В этой версии (opt5) дополнительно:
      * Окно торговли по часам (trade_start_hour / trade_end_hour, включительно).
        Фильтрует только ВХОДЫ; уже открытые позиции продолжают жить со своими
        стоп-лоссами и тейк-профитами вне зависимости от времени.
      * Запись диагностических метрик в момент сигнала входа в каждую сделку:
        - entry_ma_slope : (EMA(slow_ma_period)[0] - EMA(...)[-N]) / EMA(...)[-N]
                           где N = slow_ma_slope_window. Безразмерная доля.
        - entry_adx      : значение ADX(14)
        - entry_dc       : доминирующий цикл из AutoTune (self.atf.dc[0])
        - entry_atr_pct  : ATR(14) / close[0] (доля цены)
    """
    ...
    params = dict(
        ...
        # === Окно торговли (целые часы локального времени данных) ===
        trade_start_hour=10,   # включительно
        trade_end_hour=18,     # включительно

        # === Параметры медленной EMA для диагностики ===
        slow_ma_period=40,         # период EMA
        slow_ma_slope_window=8,    # на скольких барах меряем наклон
    )
    ...
    def __init__(self):
        ...
        # === Дополнительные индикаторы для диагностики сделок ===
        self.slow_ma = bt.indicators.EMA(self.data.close, period=self.p.slow_ma_period)
        ...
        # Метрики, фиксируемые в момент сигнала входа.
        # Хранятся между трейдами и перезаписываются на каждом новом входе;
        # анализатор читает их в notify_trade при закрытии позиции.
        self._entry_ma_slope = None
        self._entry_adx      = None
        self._entry_dc       = None
    ...
    def _within_trading_window(self):
        """
        True, если текущий бар попадает в [trade_start_hour, trade_end_hour]
        включительно (по часу локального времени самого бара).
        """
        bar_hour = self.data.datetime.datetime(0).hour
        return self.p.trade_start_hour <= bar_hour <= self.p.trade_end_hour

    def _capture_entry_metrics(self):
        """
        Снимает диагностические метрики на текущем баре (баре сигнала входа).
        Все значения — в виде, пригодном для записи в Excel: float или None.
        """
        n = int(self.p.slow_ma_slope_window)

        # Медленная MA: относительный наклон за окно n баров.
        # (MA[0] - MA[-n]) / MA[-n] — доля. Безразмерно, сравнимо между
        # инструментами с разной абсолютной ценой.
        ma_slope = None
        try:
            ma_now  = float(self.slow_ma[0])
            ma_then = float(self.slow_ma[-n])
            if (not math.isnan(ma_now)) and (not math.isnan(ma_then)) and ma_then != 0.0:
                ma_slope = (ma_now - ma_then) / ma_then
        except (IndexError, TypeError, ValueError):
            ma_slope = None
        self._entry_ma_slope = ma_slope

        # ADX(14)
        self._entry_adx = _safe_float(self.adx[0])

        # Доминирующий цикл из AutoTune
        self._entry_dc = _safe_float(self.atf.dc[0])

        # ATR(14) как доля текущей цены
        atr_pct = None
        atr_val   = _safe_float(self.atr[0])
        close_val = _safe_float(self.data.close[0])
        if atr_val is not None and close_val is not None and close_val != 0.0:
    ...
    def next(self):
        ...
        # Запрет на новые входы вне торгового окна.
        # Открытые позиции и их стоп/тейк продолжают работать независимо.
...
def main(maxcpus=None):
    ...
    params = dict( # CNY_1h - final 07-05-26
        ...
        # Окно торговли — пока фиксированное; для оптимизации заменить на range(...)
        trade_start_hour=10,
        trade_end_hour=18,
        # Медленная EMA для диагностики
        slow_ma_period=40,
        slow_ma_slope_window=8,
    )
    ...
    params = dict(  # RTS_1h
        ...
        printlog=False,
        # === Окно торговли — здесь его можно оптимизировать ===
        # Например: trade_start_hour=range(9, 13), trade_end_hour=range(15, 20)
        trade_start_hour=[9,10,11],
        trade_end_hour=[17,18,19,20,21,22],        # === EMA для диагностики (фиксируем по умолчанию) ===
        slow_ma_period=40,
        slow_ma_slope_window=8,
    )
